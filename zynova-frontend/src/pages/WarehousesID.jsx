...
export default function WarehouseDetails() {
  ...
  if (!isStockLoading && !warehouseInfo) {
    return (
      <NotFound />
    );
  }
  ...
  const tableContent = () => {
    if (isStockLoading) {
      ...
      return warehouseStock.map((stockItem, index) => (
        <tr key={index} className="border-b border-gray-200">
          <td className="p-2 flex justify-start items-center gap-4">
            <img src={stockItem.productIcon} alt={stockItem.productName} className="w-12 h-12 border-2 border-primary rounded-full" />{stockItem.productId}
          </td>
          <td className="p-2">{stockItem.productName}</td>
          <td className="p-2">{stockItem.availableStock} kg</td>
          <td className="p-2">{stockItem.fullStock} kg</td>
          <td className="p-2 flex justify-end self-center">
            <button className="bg-primary hover:bg-green-700 text-white font-bold py-2 px-4 rounded flex items-start"
              onClick={() => handleStockEdit(stockItem)} >
            ...
            <button className="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded ml-2 flex items-start"
              onClick={() => handleStockDelete(stockItem)}
            >
  ...
  return (
    <div className="flex flex-col items-start w-full p-8">
      {openWarehouseEditModal && <WarehouseEditDialog warehouse={warehouseInfo} open={openWarehouseEditModal} setOpenHook={setOpenWarehouseEditModal} />}
      {openDeleteWarehouseModal && <DeleteWarehouseDialog warehouse={warehouseInfo} open={openDeleteWarehouseModal} setOpenHook={setOpenDeleteWarehouseModal} />}
      {openAddStockModal && <AddStockDialog warehouse={warehouseInfo} open={openAddStockModal} setOpenHook={setOpenAddStockModal} />}
      {openDeleteStockModal && <DeleteStockDialog warehouse={warehouseInfo} product={selectedItem} open={openDeleteStockModal} setOpenHook={setOpenDeleteStockModal} />}
      <div className="bg-white shadow-lg rounded-lg px-8 py-6 mb-4 w-full">
        <div className="flex justify-between items-center border-b-2 pb-4 mb-4">
          ...
          <span className="text-lg font-semibold mr-4">ID: {warehouseInfo?.id || "Not Available"}</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <div>
            ...
            <p>
              {warehouseInfo?.location || "Not Available"}
            </p>
          </div>
          <div>
            <p className="font-bold">Manager:</p>
            <p>
              {warehouseInfo?.managerName || "Not Available"}
            </p>
          </div>
        </div>
      </div>
      ...
      {openStockEditModal && <StockEditDialog warehouse={warehouseInfo} stockItem={selectedItem} open={openStockEditModal} setOpenHook={setOpenStockEditModal} />}
